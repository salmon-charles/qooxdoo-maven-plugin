...
#    * Charles Salmon
#
# Based on the qooxdoo generator.py written by Sebastian Werner and Thomas Herchenroeder
#
################################################################################

################################################################
# Bellow is the code of the generator
# Compare to the original, the main() method is divided in 3 parts:
...
################################################################
# Bellow is specific code for the qonsole
# Compare to generator.py, we 
...
def qxjavainit():
    ...
    # Create job shortcut, so a job can be executed with it's name
    # Ex: clean() => jobs("clean")
    availableJobs = config.getExportedJobsList()
    for job in sorted(availableJobs):
        fjob = job.replace("-","");
        globals()[fjob] = eval("lambda : jobs(\""+job+"\")")
        console.info( "Job \'"+job+"\' => "+fjob+"()");
 
def qonsole():
    """ Start the qooxdoo interactive console, make sure to call qxjavainit before"""
    console.head(u"Qooxdoo Interactive Console", True)
